...
class Welcome(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.welcome_config = {}  # {guild_id: {channel_id, message, bg_url}}
    ...
    async def create_welcome_card(self, member):
        ...
        try:
            ...
            try:
                ...
                border_draw.ellipse((0, 0, 160, 160), outline=(114, 137, 218), width=5)
                
                # Paste avatar with border
                img.paste(border, (45, 70), border)
                img.paste(avatar, (50, 75), mask)
            except:
                # If avatar download fails, use placeholder
    ...
    async def set_welcome(self, ctx, channel: discord.TextChannel = None, *, message: str = None):
        ...
        if message is None:
            message = f"Welcome {{{{member}}}} to {{{{server}}}}! 🎉"
    ...
    async def set_goodbye(self, ctx, channel: discord.TextChannel = None, *, message: str = None):
        ...
        if message is None:
            message = f"Goodbye {{member}}! We'll miss you 😢"
